...
- **Keybind text overlay on cooldown icons** — adds a small keybind label (e.g. `1`, `S2`, `M4`) to each icon in:
  - Blizzard's built-in **Essential** and **Utility** Cooldown Viewers
...
- World of Warcraft: The War Within (Interface 12.0.0/12.0.1)
...
2. Make sure the folder is named `CooldownManagerKeybinds` (or `CooldownManagerKeybinds_Improved`, matching however you installed it) so the `.toc` file loads correctly.
3. Enable the addon at the character select screen (AddOns list) if it isn't already.

## Usage

Open the settings from **Game Menu → Options → AddOns → CooldownManagerKeybinds**, or with the slash command:

```
/cmk options
```

For each viewer group (Essential, Utility, and any BCDM/Ayije_CDM groups you have installed) you can toggle keybinds on/off and adjust:
